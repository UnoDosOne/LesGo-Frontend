...
const RequestForm = ({ isOpen, setIsOpen, data, onBack }) => {
	...
	const [gradDate, setGradDate] = useState({ day: '', month: '', year: '' });
	...
	useEffect(() => {
		const convertDateToTimestamp = () => {
			...
			const graduateDate = new Date(formData.form.gradDate || 0);
			...
			if (gradDate) {
				if (gradDate && gradDate.day) {
					graduateDate.setDate(gradDate.day + 0);
				}
				if (gradDate && gradDate.month) {
					graduateDate.setMonth(gradDate.month);
				}
				if (gradDate && gradDate.year) {
					graduateDate.setFullYear(gradDate.year);
				}
				setFormData((prevFormData) => ({
					...prevFormData,
					form: {
						...prevFormData.form,
						graduatedDate: graduateDate.getTime(),
					},
				}));
			}
		};
    ...
    if (!phoneRegex.test(phone)) {
      alert("Please enter a valid Philippine phone number (11 digits).");
		...
		if (isFormComplete) {
			try {
				...
				if (response.status === 201) {
					...
					window.location.reload(); // Optionally, reload or navigate
				}
			} catch (error) {
	...
	return (
		<div className="w-full flex flex-col justify-center items-center">
				...
				<div className="w-full">
					...
					<div className="w-full my-4 text-center">
						<p className="text-lg font-inter font-bold">
							Document Price: <span className="text-green-600">{documentPrice} PHP</span>
						</p>
					</div>
					<div className="flex flex-col w-full">
							...
							<div className="flex flex-col w-full gap-2">
								<div className="flex gap-x-2">
									<select
										...
										className="appearance-none w-full p-2 border-[2px] border-black custom-select font-inter hover:ring-2 hover:ring-black">
						...
						<div className="flex flex-col gap-y-2">
							<div className="w-full gap-2 flex">
								<div className="flex w-full gap-x-2 items-center">
									<p className="text-sm font-inter flex">
										<p
											className={`${
												formData && formData.form.graduatedDate ? 'hidden' : 'block'
											} text-red-600`}>
											*
										</p>
										Date Graduated:
									...
									<div className="flex gap-x-2 items-center">
										<input
											...
											className="font-inter border-2 border-black p-1 py-2 w-10 text-sm text-center"
										/>
										<p>/</p>
										<input
											placeholder="mm"
										...
										<input
											...
											onChange={(e) => {
												const value = e.target.value;
												if (/^\d{0,4}$/.test(value) && value <= 9999) {
													setGradDate((gradDate) => ({
														...gradDate,
											...
											className="  font-inter border-2 border-black p-1 py-2 w-12 text-sm text-center"
										/>
									</div>
								</div>
							</div>
								...
								<div className="flex flex-col w-full gap-2">
									...
									<div className="flex gap-x-2">
										<select
											...
											onChange={handleChange}
											className="w-full p-2 border-[2px] border-black custom-select font-inter hover:ring-2 hover:ring-black">
											<option value={''} className="text-gray-500">
												Select
											</option>
											{schoolYears.map((year, index) => (
												<option className="text-gray-900" key={index} value={year}>
													{year}
												</option>
											))}
										</select>
										...
										<div className="w-full sm:w-[30%] md:w-1/2">
											...
											<div className="flex gap-x-2 items-center">
												<input
													...
													className="font-inter border-2 border-black p-1 py-2 w-full sm:w-10 text-sm text-center"
												/>
												<p>/</p>
												<input
													placeholder="mm"
												...
												<input
													...
													value={reqDate.year}
													className="  font-inter border-2 border-black p-1 py-2 w-full  text-sm text-center"
												/>
											</div>
										</div>
									</div>
							...
							<div className="flex flex-col w-full gap-2">
								...
								<div className="flex">
									<div className=" border-t-2 border-b-2 border-l-2 border-black font-inter h-11 items-center flex px-2">
										+63
									</div>
									<input
										type="number"
										...
										className="w-full p-2 border-2 border-black font-inter hover:ring-2 hover:ring-black"
									/>
								</div>
							</div>
						</div>
						...
						<div className="w-full gap-2 flex">
                          ...
                          w-full"
									...
									{formData.form.clearanceStatus === 'No' && (
										<p className="text-center p-2 font-inter text-gray-900 text-sm border-2 w-full border-black cursor-pointer hover:underline">
			...
			<div className="w-full py-4 flex justify-center items-center  gap-x-6 translate-y-8">
				<button
					onClick={onBack}
